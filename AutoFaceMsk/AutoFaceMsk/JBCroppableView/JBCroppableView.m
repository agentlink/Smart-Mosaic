...
- (UIImage *)maskImageFromFile:(CGRect)imageRect isEarse:(BOOL)earse withImageFrame:(CGRect)imageFrame withScalePoint:(CGPoint)scalePoint
{
    ...
    
    {
        ...
        [aPath moveToPoint:CGPointMake(p1.x - imageFrame.origin.x + scalePoint.x, rect.size.height - p1.y + scalePoint.y - imageFrame.origin.y)];
        ...
        for (uint i=1; i<points.count; i++)
        {
            ...
            [aPath addLineToPoint:CGPointMake(p.x - imageFrame.origin.x + scalePoint.x, rect.size.height - p.y + scalePoint.y - imageFrame.origin.y)];
            [aPath stroke];
        }
        [aPath closePath];
        [aPath fill];
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if ((CGRectContainsPoint(CGRectMake(0, 0, self.frame.size.width, self.frame.size.height), point))) {
        ...
        isContainView = [LastBezierPath containsPoint:CGPointMake(point.x, point.y)];
